...
from utils import TransferCIFAR10Pair, CIFAR10Pair, TransferCIFAR100Pair, CIFAR100Pair, PoisonTransferCIFAR10Pair, LinearModelData
...
if __name__ == '__main__':
    ...
    if args.pre_load_name == '':
        ...
        save_name_pre = args.pre_load_name + "_supervised_cleantrain_class{}".format(args.num_class)
    else:
        perturb_tensor_filepath="./results/{}.pt".format(args.pre_load_name)
        save_name_pre = args.pre_load_name + "_supervised_class{}".format(args.num_class)

    if args.train_data_type == 'cifar10':
        trainset = LinearModelData(root='data', train=True, transform=transform_test, download=True, perturb_tensor_filepath=perturb_tensor_filepath)
        trainloader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, num_workers=2)
        testloader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=False, num_workers=2)
    ...
    net = LinearModel([3, 32, 32], args.num_class)
    ...
    if not args.test_dbindex:
        if args.just_test:
            ...
            for epoch in range(start_epoch, start_epoch+50):
                ...
                test(epoch, optimizer, save_name_pre)
                scheduler.step()
    else:
        test_cluster()
