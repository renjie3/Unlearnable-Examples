        ...
        while condition:
            ...
            for i, (pos_samples_1, pos_samples_2, labels) in tqdm(enumerate(train_noise_data_loader_simclr), total=len(train_noise_data_loader_simclr), desc="Training images"):
                ...
                if args.attack_type == 'min-min':
                    # print(pos_samples_1.cpu().numpy()[0][0])
                    perturb_img, eta, train_noise_loss = noise_generator.min_min_attack_simclr_large_noise(pos_samples_1, pos_samples_2, labels, model, optimizer, None, random_noise=batch_noise, batch_size=batch_size, temperature=temperature)
...
def main():
    ...
    print("This gives the same augmentation in image pair.")
    ...
    train_noise_data = utils.SameImgCIFAR10Pair(root='data', train=True, transform=utils.train_transform, download=True)
    train_noise_data.replace_random_noise_class(random_noise_class)
    train_noise_data_loader = DataLoader(train_noise_data, batch_size=batch_size, shuffle=False, num_workers=0, pin_memory=True)
    ...
    if args.attack_type == 'random':
        ...
        if args.perturb_type == 'samplewise':
            ...
            # noise = universal_perturbation(noise_generator, None, None, model, None, optimizer, None, random_noise, ENV, train_loader, train_noise_data_loader, batch_size, temperature, memory_loader, test_loader, k)
            noise = universal_perturbation(noise_generator, trainer=None, evaluator=None, model=model, criterion=None, optimizer=optimizer, scheduler=None, random_noise=random_noise, ENV=ENV, train_loader_simclr=train_loader, train_noise_data_loader_simclr=train_noise_data_loader, batch_size=batch_size, temperature=temperature, memory_loader=memory_loader, test_loader=test_loader, k=k)
        torch.save(noise, os.path.join(args.exp_name, 'perturbation.pt'))
        logger.info(noise)
        logger.info(noise.shape)
        logger.info('Noise saved at %s' % (os.path.join(args.exp_name, 'perturbation.pt')))
