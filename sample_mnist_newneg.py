...
padding_size = [[0, 0, 32 - size, 32 - size],
                ...
                [32 - size, 32 - size, 0, 0],
                [0, 32 - size, 32 - size, 0],]
center_padding = [(32 - size) // 2, (32 - size) // 2, (33 - size) // 2, (33 - size) // 2]
...
for i in range(idx, test_mnist.shape[0]):
    ...
    # label = i % 4
    # padding = padding_size[label]
    img = ImageOps.expand(img, border=(center_padding[0],center_padding[1],center_padding[2],center_padding[3]), fill=0)##left,top,right,bottom
    img = np.array(img)
    # if k < 256:
    #     label = 0
    #     img = img * 0.25
...
file_path = './data/sampled_cifar10/mnist_train_4class_4batch_4_grayposit.pkl'
