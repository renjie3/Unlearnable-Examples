...
from methods.knn import knn_monitor
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, train_loader_simclr, train_noise_data_loader_simclr, batch_size, temperature, memory_loader, test_loader, k, train_data_for_save_img, save_name_pre, const_train_loader, g_net,  supervised_criterion, supervised_optimizer, supervised_scheduler, supervised_transform_train):
    ...
    # test_acc_1 = knn_monitor(model.backbone, memory_loader, test_loader, 0, k, 0.1, hide_progress=False, classes=10)
    # test_acc_5 = 0
    ...
    for _epoch_idx in range(1, epochs+1):
        ...
        for pos_samples_1, pos_samples_2, labels in tqdm(train_loader_simclr, total=len(train_loader_simclr)):
            ...
            model.train()
            batch_train_loss, batch_size_count = train_simsiam(model, pos_samples_1, pos_samples_2, optimizer, batch_size, temperature, noise_after_transform=args.noise_after_transform_train_model, pytorch_aug=False)
        ...
        # test_acc_1 = knn_monitor(model.backbone, memory_loader, test_loader, epoch_idx, k, 0.1, hide_progress=False, classes=10)
        # test_acc_5 = 0
...
def main():
    ...
    if args.cl_algorithm == 'simclr':
        ...
        model = model.cuda()
    elif args.cl_algorithm == 'simsiam':
