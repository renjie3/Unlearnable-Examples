...
def universal_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, train_loader_simclr, train_noise_data, batch_size, temperature, memory_loader, test_loader, k):
    ...
    for epoch_idx in range(1, epochs+1):
        ...
        while condition:
            ...
            train_noise_data_loader_simclr = DataLoader(train_noise_data, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True)
            for i, (pos_samples_1, pos_samples_2, labels) in tqdm(enumerate(train_noise_data_loader_simclr), total=len(train_noise_data_loader_simclr), desc="Training images"):
                ...
                if args.attack_type == 'min-min':
                    perturb_img, eta, train_noise_loss = noise_generator.min_min_attack_simclr_return_loss_tensor(pos_samples_1, pos_samples_2, labels, model, optimizer, None, random_noise=batch_noise, batch_size=batch_size, temperature=temperature)
            ...
            del train_noise_data_loader_simclr
        ...
        if epoch_idx % 10 == 0:
            ...
            torch.save(noise, 'results/{}_check_point_perturbation.pt'.format(save_name_pre))
...
def main():
    ...
    if args.attack_type == 'random':
        ...
        if args.perturb_type == 'samplewise':
            ...
            noise, save_name_pre = universal_perturbation(noise_generator, None, None, model, None, optimizer, None, random_noise, ENV, train_loader, train_noise_data, batch_size, temperature, memory_loader, test_loader, k)
