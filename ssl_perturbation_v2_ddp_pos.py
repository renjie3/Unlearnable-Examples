    ...
    for _epoch_idx in range(1, epochs+1):
        ...
        while condition:
            ...
            for i, (pos_samples_1, pos_samples_2, labels) in tqdm(enumerate(train_noise_data_loader_simclr), total=len(train_noise_data_loader_simclr), desc="Training images"):
                ...
                if args.attack_type == 'min-min':
                    if args.min_min_attack_fn == "eot_v1":
                        _, eta, train_noise_loss = noise_generator.min_min_attack_simclr_return_loss_tensor_eot_v1_no_eval_pos_only(pos_samples_1, pos_samples_2, labels, model, optimizer, None, random_noise=batch_noise, batch_size=batch_size, temperature=temperature, flag_strong_aug=args.strong_aug, noise_after_transform=args.noise_after_transform, eot_size=args.eot_size, one_gpu_eot_times=args.one_gpu_eot_times, cross_eot=args.cross_eot, pytorch_aug=args.pytorch_aug, dbindex_weight=args.dbindex_weight, simclr_weight=args.simclr_weight)
                    elif args.min_min_attack_fn == "non_eot":
