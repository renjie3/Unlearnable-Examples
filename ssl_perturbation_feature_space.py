...
parser.add_argument('--save_clean_feature_space', action='store_true', default=False)
parser.add_argument('--pre_load_name', default='', type=str, help='The model parameters that we are going to load.')
                ...
                for j in range(0, args.train_step):
                    ...
                    for idx_model, model in enumerate(model_group):
                        ...
                        for param in model.parameters():
                            param.requires_grad = True
                        # trainer.train_batch(torch.stack(train_imgs).to(device), labels, model, optimizer)
                        batch_train_loss, batch_size_count = train_simclr(model, torch.stack(train_pos_1).to(device), torch.stack(train_pos_2).to(device), optimizer[idx_model], batch_size, temperature)
                        sum_train_loss += batch_train_loss
                        sum_train_batch_size += batch_size_count
                        break
...
def save_clean_feature_space(net, save_name_pre, clean_trained_data):
    if args.no_save:
        raise("Wrong. Please remove --no_save.")
    net.eval()
    feature_bank = []
    with torch.no_grad():
        clean_trained_data_loader = DataLoader(clean_trained_data, batch_size=args.batch_size, shuffle=False, num_workers=16, pin_memory=True)
        # generate feature bank
        for data, _, target in tqdm(clean_trained_data_loader, desc='Feature extracting'):
            feature, out = net(data.cuda(non_blocking=True))
            feature_bank.append(feature)
        # [D, N]
        feature_bank = torch.cat(feature_bank, dim=0).contiguous()
    torch.save(feature_bank, './results/{}_clean_trained_feature_space.pt'.format(save_name_pre))
    logger.info("Save clean feature space done.")
    # print(type(feature_bank.shape))
    logger.info("The shape is {}.".format(' '.join(str(elem) for elem in list(feature_bank.shape))))

def plot_distribution(feature_bank, targets, pre_load_name):
    ...
    org_feature = feature_tsne_output[:feature_bank.shape[0] // 2]
    perturb_feature = feature_tsne_output[feature_bank.shape[0] // 2:]
    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(1, 1, 1)
    plt.title("feature space")
    plt.scatter(org_feature[:, 0], org_feature[:, 1], s=10, c=labels_tsne_color, marker='o', cmap=plt.cm.Spectral)
    plt.scatter(perturb_feature[:, 0], perturb_feature[:, 1], s=10, c=labels_tsne_color, marker='v', cmap=plt.cm.Spectral)
    ax.xaxis.set_major_formatter(NullFormatter())  # 设置标签显示格式为空
    ax.yaxis.set_major_formatter(NullFormatter())
    plt.savefig(pre_load_name + '_feature_space.png')
    plt.close()

def feature_space_perturbation(noise_generator, group_model, save_name_pre, org_image_dataset, random_noise, clean_feature_space):
    
    org_image_dataset_loader = DataLoader(org_image_dataset, batch_size=args.batch_size, shuffle=False, num_workers=16, pin_memory=True)
    target_feature_space_batches = torch.split(clean_feature_space, args.batch_size, dim=0)

    mask_cord_list = []
    idx = 0
    for images, labels in org_image_dataset_loader:
        for i, (image, label) in enumerate(zip(images, labels)):
            noise = random_noise[idx]
            mask_cord, _ = noise_generator._patch_noise_extend_to_img(noise, image_size=image.shape, patch_location=args.patch_location)
            mask_cord_list.append(mask_cord)
    ...
    for epoch_idx in range(1, epochs+1):
        ...
        idx = 0
        loss_sum = 0
        group_feature = [[] for _ in range(len(group_model))]
        for batch_idx, (images, labels) in tqdm(enumerate(org_image_dataset_loader), total=len(org_image_dataset_loader)):
            for model_idx, model in enumerate(group_model):
                group_model[model_idx] = model.to(device)
            images, labels = images.to(device), labels.to(device)

            # Construct the batch of sample_wise noise
            batch_noise, batch_start_idx = [], idx
            for i, (image, label) in enumerate(zip(images, labels)):
                sample_noise = random_noise[idx]
                c, h, w = image.shape[0], image.shape[1], image.shape[2]
            ...
            # Update feature_space perturbation
            # model.eval()
            # for param in model.parameters():
            #     param.requires_grad = False
            batch_noise = torch.stack(batch_noise).to(device)
            perturb_img, eta, loss_batch, group_feature_batch = noise_generator.feature_space_distribution_attack(group_model=group_model, images=images, target_feature_space=target_feature_space_batches[batch_idx], random_noise=batch_noise)
            loss_sum += loss_batch
            for i in range(len(group_feature_batch)):
                group_feature[i].append(group_feature_batch[i])
        ...
        print("feature loss: ", loss_sum * 255 / 1024.0 * args.batch_size / len(group_model))
        if epoch_idx % 5 == 1:
        # if True:
            for i in range(len(group_feature)):
                group_feature[i] = torch.cat(group_feature[i], dim=0)
                # print(group_feature[i].shape)
            
            if args.job_id == '':
                feature_visual_path = './feature_space_images/local_{}_{}_{}_{}_feature_distribution/'.format(args.pre_load_name, epsilon_255, args.num_steps, step_size_255)
            else:
                feature_visual_path = './feature_space_images/{}_{}_{}_{}_{}_feature_distribution/'.format(args.job_id, args.pre_load_name, epsilon_255, args.num_steps, step_size_255)
            input_feature = torch.cat([clean_feature_space, group_feature[0]])
            if not os.path.exists(feature_visual_path):
                os.mkdir(feature_visual_path)
            plot_distribution(input_feature, org_image_dataset.targets, feature_visual_path + str(epoch_idx))
            torch.save(random_noise, './results/{}_{}_{}_{}_{}_perturb_on_random_initial_model.pt'.format(args.pre_load_name, args.job_id, epsilon_255, args.num_steps, step_size_255))
...
def main():
    ...
    org_image_dataset = utils.SampledCIFAR10(root='data', train=True, transform=utils.ToTensor_transform, download=True)
    ...
    if args.save_clean_feature_space:
        model = Model(feature_dim, arch=args.arch).cuda()
    ...
    if args.save_clean_feature_space:
        ...
        save_clean_feature_space(model, pre_load_name, train_data)

    noise = noise_generator.random_noise(noise_shape=args.noise_shape)
    clean_feature_space_path = './results/{}_clean_trained_feature_space.pt'.format(args.pre_load_name)
    clean_feature_space = torch.load(clean_feature_space_path)
    target_feature_space_batches = torch.split(clean_feature_space, args.batch_size, dim=0)

    feature_space_perturbation(noise_generator, model, args.pre_load_name, org_image_dataset, noise, clean_feature_space)
