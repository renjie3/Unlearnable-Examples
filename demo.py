...
data_num = 10
iteration_step = 50
class_norm_center = [[1,3],[-5,4],[8,-3]]
feat_norm_center = [-4, 1, 5]
use_delta = False
...
for i in range(class_num):
    ...
    feat = np.random.normal(loc=feat_norm_center[i], scale=3, size=10)
...
if use_delta:
    ...
    for _step in range(iteration_step):
        ...
        for i in range(class_num * data_num):
            ...
            for j in range(class_num * data_num):
                ...
                if i != j:
                    ...
                    temp_force = (np.exp(np.dot(last_input[i], last_input[j])) - np.exp(get_length(last_input[i] - last_input[j]))) * 0.01
        ...
        for i in range(class_num * data_num):
            ...
            adjusted_feature.append(last_feature[i] + 0.05 * changed_feat)
            # print(changed_feat)
        ...
        for i in range(class_num * data_num):
            ...
            new_pos = adjusted_data1[i] + 0.2 * avg_neighbor
        ...
        for i in range(class_num):
            ...
            plt.scatter(last_input[i*data_num:(i+1)*data_num, 0], last_input[i*data_num:(i+1)*data_num, 1], s=10, c=color_list[i], cmap=plt.cm.Spectral)
            plt.xlim((-1.2, 1.2))
            plt.ylim((-1.2, 1.2))
