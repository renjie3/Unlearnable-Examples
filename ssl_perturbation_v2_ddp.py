    ...
    for epoch_idx in range(1, epochs+1):
        ...
        while condition:
            ...
            for i, (pos_samples_1, pos_samples_2, labels) in tqdm(enumerate(train_noise_data_loader_simclr), total=len(train_noise_data_loader_simclr), desc="Training images"):
                ...
                eta_list = [torch.zeros_like(eta) for _ in range(args.gpu_num)]
                torch.distributed.all_gather(eta_list, eta)

                labels_list = [torch.zeros_like(labels) for _ in range(args.gpu_num)]
                torch.distributed.all_gather(labels_list, labels)

                for _eta, _labels in zip(eta_list, labels_list):
                    for delta, label in zip(_eta, _labels):
                        # x1, x2, y1, y2 = mask_cord_list[label.item()]
                        # delta = delta[:, x1: x2, y1: y2]
                        if torch.is_tensor(random_noise):
                            random_noise[label.item()] = delta.detach().cpu().clone()
                        else:
                            random_noise[label.item()] = delta.detach().cpu().numpy()

            cuda_random_noise = random_noise.cuda()

            random_noise_list = [torch.zeros_like(cuda_random_noise) for _ in range(args.gpu_num)]
            torch.distributed.all_gather(random_noise_list, cuda_random_noise)

            random_noise = random_noise_list[0].detach().cpu()
    ...
    for epoch_idx in range(1, epochs+1):
        ...
        while condition:
            ...
            for i, (pos_samples_1, pos_samples_2, labels) in tqdm(enumerate(train_noise_data_loader_simclr), total=len(train_noise_data_loader_simclr), desc="Training images"):
                ...
                eta_list = [torch.zeros_like(eta.shape) for _ in range(args.gpu_num)]
                torch.distributed.all_gather(eta_list, eta)

                labels_list = [torch.zeros_like(labels.shape) for _ in range(args.gpu_num)]
                torch.distributed.all_gather(labels_list, labels)
...
def main():
    ...
    if args.perturb_type == 'theory_model':
        ...
        train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=flag_shuffle_train_data, num_workers=args.num_workers, pin_memory=True, drop_last=False, sampler=train_sampler)
        ...
        train_noise_sampler = DistributedSampler(train_noise_data)
        train_noise_data_loader = DataLoader(train_noise_data, batch_size=batch_size, shuffle=args.shuffle_train_perturb_data, num_workers=args.num_workers, pin_memory=True, sampler=train_noise_sampler)
        # test data don't have to change the target. by renjie3
