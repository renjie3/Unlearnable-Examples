...
if __name__ == '__main__':
    ...
    train_data = utils.TransferCIFAR10Pair(root='data', train=True, transform=utils.ToTensor_transform, download=True, perturb_tensor_filepath="./results/{}.pt".format(pre_load_name), random_noise_class_path=random_noise_class_path, perturbation_budget=0, class_4=class_4, samplewise_perturb=samplewise_perturb, org_label_flag=args.orglabel)
    if not os.path.exists('./images/{}'.format(save_name_pre)):
        os.mkdir('./images/{}'.format(save_name_pre))
    utils.save_img_group(train_data, train_data.perturb_tensor.to('cpu'), './images/{}/{}.png'.format(save_name_pre, 0))
    ...
    for epoch in range(1, epochs + 1):
        train_loss = 0 #train(model, train_loader, optimizer)
