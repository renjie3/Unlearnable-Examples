...
parser.add_argument('--single_corner', action='store_true', default=False)
...
for i in range(len(train_data.data)):
    ...
    if target in idx_dict:
        idx = idx_dict[target]
        mnist_idx = mnist_single_idx_dict[target]
        if len(train_mnist_img_list[idx]) < 256:
            train_mnist_img_list[idx].append(np.stack([train_data.data[mnist_idx].cpu().numpy() for _ in range(3)], axis=2))
...
for i in range(last_i, len(train_data.data)):
    ...
    if target in idx_dict:
        ...
        if len(test_mnist_img_list[idx]) < 1000:
            test_mnist_img_list[idx].append(np.stack([train_data.data[mnist_idx].cpu().numpy() for _ in range(3)], axis=2))
...
for i in range(4):
    for j in range(len(train_mnist_img_list[i])):
        ...
        if not args.center:
            if args.single_corner:
                padding_corner = (padding[0][0], padding[0][1], padding[0][2], padding[0][3])
            else:
...
if args.center:
    if args.train:
        file_path = './data/sampled_cifar10/cifar10_1024_4class_mnist_mixed_center_train_{}_budget{}.pkl'.format(mnist_size, multi_budget*8)
    else:
        file_path = './data/sampled_cifar10/cifar10_1024_4class_mnist_mixed_center_all_{}_budget{}.pkl'.format(mnist_size, multi_budget*8)
else:
    if args.single_corner:
        if args.train:
            file_path = './data/sampled_cifar10/cifar10_1024_4class_mnist_mixed_single_corner_train_{}_budget{}.pkl'.format(mnist_size, multi_budget*8)
        else:
            file_path = './data/sampled_cifar10/cifar10_1024_4class_mnist_mixed_single_corner_all_{}_budget{}.pkl'.format(mnist_size, multi_budget*8)
    else:
        if args.train:
            file_path = './data/sampled_cifar10/cifar10_1024_4class_mnist_mixed_corner_train_{}_budget{}.pkl'.format(mnist_size, multi_budget*8)
        else:
            file_path = './data/sampled_cifar10/cifar10_1024_4class_mnist_mixed_corner_all_{}_budget{}.pkl'.format(mnist_size, multi_budget*8)
