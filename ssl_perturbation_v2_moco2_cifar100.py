...
from moco import ModelMoCo, train_moco, test_moco
...
def universal_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, train_loader_simclr, train_noise_data_loader_simclr, batch_size, temperature, memory_loader, test_loader, k, train_data_for_save_img, const_train_loader, save_name_pre):
    ...
    for _epoch_idx in range(1, epochs+1):
        ...
        test_acc_1, test_acc_5 = test_moco(model,encoder_q, memory_loader, test_loader, epoch_idx, epochs, k, temperature)
...
def sample_wise_perturbation(noise_generator, trainer, evaluator, model, criterion, optimizer, scheduler, random_noise, ENV, train_loader_simclr, train_noise_data_loader_simclr, batch_size, temperature, memory_loader, test_loader, k, train_data_for_save_img, save_name_pre, const_train_loader, g_net,  supervised_criterion, supervised_optimizer, supervised_scheduler, supervised_transform_train):
    ...
    test_acc_1 = test_moco(model.encoder_q, memory_loader, test_loader, 0, epochs, k, temperature, )
    # test_moco(net, memory_data_loader, test_data_loader, epoch, epochs, knn_k, knn_t)
    ...
    for _epoch_idx in range(1, epochs+1):
        ...
        train_loader_simclr.dataset.update_perturb_to_data(random_noise)
        # input()
            ...
            if epoch_idx != 1 or not args.train_perturb_fisrt:
                if args.attack_type == 'min-min':
                    ...
                    for j in range(0, args.train_step):
                        ...
                        for param in model.parameters():
                            param.requires_grad = True
                        batch_train_loss, batch_size_count = train_moco(model, pos_samples_1.to(device), pos_samples_2.to(device), optimizer)
        ...
        print(train_loss)
        results['train_loss'].append(train_loss)
        test_acc_1 = test_moco(model.encoder_q, memory_loader, test_loader, epoch_idx, epochs, k, temperature)
        results['test_acc@1'].append(test_acc_1)
...
def main():
    ...
    if args.train_data_type == 'CIFAR10':
        train_data = utils.PytorchAugCIFAR10Pair(root='data', train=True, transform=utils.train_transform, download=True, class_4=args.class_4, train_noise_after_transform=args.noise_after_transform_dataset, kmeans_index=args.kmeans_index, unlearnable_kmeans_label=args.unlearnable_kmeans_label, kmeans_label_file=args.kmeans_label_file)
        train_data.replace_targets_with_id()
        train_loader = DataLoader(train_data, batch_size=batch_size, shuffle=flag_shuffle_train_data, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        ...
        train_noise_data_loader = DataLoader(train_noise_data, batch_size=batch_size, shuffle=args.shuffle_train_perturb_data, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        ...
        plot_input_data = utils.CIFAR10Pair(root='data', train=False, transform=utils.train_transform, download=True, class_4=args.class_4, train_noise_after_transform=False, gray=args.gray_test, class_4_train_size=args.class_4_train_size)
        ...
        train_data = utils.PytorchAugCIFAR100Pair(root='data', train=True, transform=utils.ToTensor_transform, download=True, train_noise_after_transform=args.noise_after_transform_dataset, kmeans_index=args.kmeans_index, unlearnable_kmeans_label=args.unlearnable_kmeans_label)
        train_data.replace_targets_with_id()
